  ...
  broadcaster_(this)
{
  ...
  declare_parameter("robot_height", 0.4);  // default 0.4 m
  get_parameter("robot_height", this->robot_height_);
  ...
  const rclcpp::Time stamp)
{
  ...
  // ↓↓↓ New z‐cut filter to drop all points below robot_height_ ↓↓↓

  pcl::PassThrough<pcl::PointXYZI> pass;
  pass.setInputCloud(filtered_cloud_ptr);
  pass.setFilterFieldName("z");
  pass.setFilterLimits(static_cast<float>(robot_height_),
  ...
  publishMapAndPose(filtered_cloud_ptr, final_transformation, stamp);
