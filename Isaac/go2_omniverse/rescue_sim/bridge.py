...
CycloneDDS protocol, through the team's D1 driver (`maps/d1_driver`). The sim runs that same bridge with
...
- The driver: `d1_driver/` beside the bridge script, which imports it from there.
...
DRIVER_BESIDE_BRIDGE = Path("d1_driver/arm_control.py")   # what arm_bridge.py imports D1Arm from
...
class ArmBridge:
    ...
    def __init__(self, script: Path, host: str = DEFAULT_HOST, port: int = DEFAULT_PORT, log_path: Path = DEFAULT_LOG):
        self.script, self.host, self.port = Path(script), host, port
    ...
    def resolve(cls, choice: str, port: int = DEFAULT_PORT, domain_id: int = 0):
        ...
        driver = script.parent / DRIVER_BESIDE_BRIDGE
        if not driver.is_file():
            return None, (f"not started: no D1 driver at {driver}. That VIP-Rescue checkout predates the driver "
                          f"moving into maps/d1_driver (it was the Docker/unitree-d1-control submodule)")
        return cls(script, port=port), None
