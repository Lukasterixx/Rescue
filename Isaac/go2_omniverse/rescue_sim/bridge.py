...
If something already listens on the port (the website's own bridge, or one left by an earlier sim), the sim starts
none: that bridge serves the arm, since it too talks to whatever D1 is on DDS domain 0.
...
class ArmBridge:
    ...
    def start(self, wait_s: float = 8.0) -> bool:
        ...
        threading.Thread(target=self._echo, args=(self.process,), daemon=True).start()
        deadline = time.monotonic() + wait_s
        while time.monotonic() < deadline:
            if self.process.poll() is not None:
                say(f"exited at startup with code {self.process.returncode}; the arm has no bridge")
                return False
            if listening(self.host, self.port):
                say(f"up on {self.host}:{self.port} (pid {self.process.pid}): {self.script}, driver {self.driver}")
                return True
            time.sleep(0.2)
    ...
    @staticmethod
    def _echo(process: subprocess.Popen) -> None:
